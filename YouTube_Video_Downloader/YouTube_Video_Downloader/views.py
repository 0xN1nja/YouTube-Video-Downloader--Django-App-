...
def analyze_to_mp3(request):
    ...
    try:
        ...
        params={"status":"Successfully Downloaded !","location":f"{os.path.join(BASE_DIR,yt.title)}.mp3","Format":"MP3","Title":yt.title,"Description":yt.description,"Rating":f"{yt.rating}","Duration":f"{yt.length} Seconds"}
    except:
